...
var orm = {
  ...
  selectAll: function(callback) {
    ...
    // Run MySQL Query
    connection.query('SELECT * FROM cake', function (err, result) {
      if (err) throw err;
      callback(result);
    });
  ...
  insertOne: function(cake_name, callback){
    ...
    // Run MySQL Query
    connection.query('INSERT INTO cake SET ?', {
      cake_name: cake_name,
      devoured: false,
      date: timestamp
    }, function (err, result) {
  ...
  // updateOne()
  updateOne: function(cakeID, callback){

    // Run MySQL Query
    connection.query('UPDATE cake SET ? WHERE ?', [{devoured: true}, {id: cakeID}], function (err, result) {
        if (err) throw err;
        callback(result);
      });
